...
1. Open the Course Catalog online and wait for `Доступно офлайн`.
2. Use **Share → Add to Home Screen** and confirm the `P.` icon and
   `Prosto.Courses` identity are legible.
3. Launch the saved application. Confirm it opens at the Course Catalog in
   standalone display and no installation action remains in the global control.
4. Follow internal links and confirm they remain in the installed application.
5. Open an External Reference and confirm Safari opens it separately.
6. Enable airplane mode, relaunch, and complete a Knowledge Check, update Lesson
...
1. Open the Course Catalog online and wait for `Доступно офлайн`.
2. Use **File → Add to Dock** and confirm the `P.` icon and application identity.
3. Launch from the Dock. Confirm standalone display, the Course Catalog start
   destination, in-scope internal navigation, and separately opened External
   References.
4. Disconnect networking and confirm the complete saved Course Catalog and
